  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      function ArchiveModal(props) {
        ...
        // Prune selection entries whose sessions are no longer archived
        // (deleted here or elsewhere) so the batch button never targets ghosts.
        React.useEffect(() => {
          setSelected((prev) => {
            let changed = false;
            const next = new Set();
            for (const id of prev) {
              if (archivedIds.indexOf(id) !== -1) next.add(id);
              else changed = true;
            }
        ...
        const selectedIds = archivedIds.filter((id) => selected.has(id));
        const isBulkBusy = busy === "all" || busy === "selected" || busy === "restore-all" || busy === "restore-selected";
        ...
        const doDeleteSelected = async () => {
          const ids = archivedIds.filter((id) => selected.has(id));
          if (ids.length === 0) return;
          if (!window.confirm("确认彻底删除所选 " + ids.length + " 个归档会话？\n磁盘文件与记录将一并移除，无法恢复！")) return;
          setBusy("selected");
          setError(null);
          try {
            const body = await postJson("/api/dsh-archive/delete-selected", { sessionIds: ids });
            setBusy(null);
            setSelected(new Set());
            setToast("已彻底删除 " + (body && typeof body.removed === "number" ? body.removed : ids.length) + " 个归档会话。");
          } catch (reason) {
            setBusy(null);
            setError(reason && reason.message ? reason.message : String(reason));
          }
        ...
        const doRestoreAll = async () => {
          if (archivedIds.length === 0) return;
          if (!window.confirm("确认恢复全部 " + archivedIds.length + " 个归档会话？\n会话将重新出现在会话列表中。")) return;
          setBusy("restore-all");
          setError(null);
          try {
            const body = await postJson("/api/dsh-archive/unarchive-all", {});
            setBusy(null);
            setToast("已恢复 " + (body && typeof body.restored === "number" ? body.restored : archivedIds.length) + " 个归档会话。");
          } catch (reason) {
            setBusy(null);
            setError(reason && reason.message ? reason.message : String(reason));
          }
        ...
        const doRestoreSelected = async () => {
          ...
          setError(null);
          try {
            const body = await postJson("/api/dsh-archive/unarchive-selected", { sessionIds: ids });
            setBusy(null);
            setToast("已恢复 " + (body && typeof body.restored === "number" ? body.restored : ids.length) + " 个归档会话。");
          } catch (reason) {
            setBusy(null);
            setError(reason && reason.message ? reason.message : String(reason));
          }
            ...
            h("div", { className: "dap-modal", role: "dialog", "aria-modal": "true", "aria-label": "归档会话" },
              ...
              archivedIds.length > 0
                ? h("div", { className: "dap-filterRow" },
                    h("label", { className: "dap-checkAll", title: "全选 / 取消全选当前列表中的会话" },
                      h("input", {
                        type: "checkbox",
                        className: "dap-check",
                    ...
                    h("button", {
                      type: "button",
                      className: "dap-btn",
                      disabled: isBulkBusy || archivedIds.length === 0,
                      title: selectedIds.length > 0 ? "恢复勾选的 " + selectedIds.length + " 个归档会话" : "恢复所有归档会话到会话列表",
                      onClick: selectedIds.length > 0 ? doRestoreSelected : doRestoreAll
                    },
                      (busy === "restore-all" || busy === "restore-selected")
                        ? "恢复中…"
                        : selectedIds.length > 0 ? "恢复所选" : "全部恢复"
                    ),
                    h("button", {
                      type: "button",
                      className: "dap-btn dap-btnDanger",
                      disabled: isBulkBusy || archivedIds.length === 0,
                      title: selectedIds.length > 0 ? "删除勾选的 " + selectedIds.length + " 个归档会话" : "清空所有归档会话",
                      onClick: selectedIds.length > 0 ? doDeleteSelected : doDeleteAll
                    },
                      (busy === "all" || busy === "selected")
                        ? "删除中…"
                        : selectedIds.length > 0 ? "删除所选会话" : "全部删除"
                    )
                  )
                : null,
              ...
              rows.length > 0
                ? h("p", { className: "dap-hint" }, "「删除」会永久清除会话日志与记录，且无法恢复。勾选会话后可批量「恢复所选」或「删除所选会话」；「全部恢复」「全部删除」作用于所有归档会话。")
                : null,
